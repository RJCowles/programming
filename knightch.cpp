...
void printStrings(std::vector<char> charString, std::vector<int> intString)
{
    ...
    {
        std::cout << charString.at(i) << intString.at(i);
...
{
    // x and y mean position of knight
    ...
    else
    {
        ...
        for (int i = 0; i < 8; i++)
        {
            int newPosX = x + xMove.at(i);
            int newPosY = y + yMove.at(i);
            if (in_bounds(newPosX, newPosY, boardSize))
            {
                // check how to do this with vec.at
                if (matrix.at(newPosX).at(newPosY) == 0)
                {
...
int main(int argc, char *argv[])
{
    ...
    try
    {
        if (argc == 4)
        {
            ...
            if (!check_correct_2_3_input(startPos.at(0)) || !check_correct_2_3_input(endPos.at(0)))
            {
                throw std::runtime_error("wrong input for spot 2 and 3");
            }
            int x = startPos.at(0) - 97;
            int y = stoi(startPos.substr(1, 3)) - 1;
            int endPosX = endPos.at(0) - 97;
            ...
            // to check if knights tour failed or succeded
